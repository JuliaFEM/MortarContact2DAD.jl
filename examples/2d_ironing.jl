...
tool = Problem(mesh, HyperElasticity, "TOOL", 2)
...
block = Problem(mesh, HyperElasticity, "BLOCK", 2)
...
update!(bc2, "displacement 2", 1.8 => -1.8)
update!(bc2, "displacement 1", 11.4 => 9.60)
update!(bc2, "displacement 2", 11.4 => -1.8)
...
#add_problems!(analysis, [tool, block, bc1, bc2])
add_problems!(analysis, [tool, block, bc1, bc2, contact])
#push!(block.postprocess_fields, "stress")
#push!(tool.postprocess_fields, "stress")
for body in (block, tool)
    push!(body.postprocess_fields, "small strain (volume averaged)")
    push!(body.postprocess_fields, "finite strain (volume averaged)")
    push!(body.postprocess_fields, "true stress (volume averaged)")
    push!(body.postprocess_fields, "principal stress 1 (volume averaged)")
    push!(body.postprocess_fields, "principal stress 1 direction (volume averaged)")
    push!(body.postprocess_fields, "principal stress 2 (volume averaged)")
    push!(body.postprocess_fields, "principal stress 2 direction (volume averaged)")
    push!(body.postprocess_fields, "von mises stress (volume averaged)")
end

xdmf = Xdmf(joinpath(datadir, "2d_ironing_1800_neohookean"); overwrite=true)
add_results_writer!(analysis, xdmf)

end_time = 13.2
#timespan = range(0.0, stop=end_time, length=Int(end_time*60))
timespan = 0.0:0.025:1.8
