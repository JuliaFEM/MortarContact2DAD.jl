...
function assemble_interface_2!(problem::Problem{Contact2DAD}, assembly::Assembly,
    ...
    function nasse_factory(node_id)
        ...
        function F!(y::AbstractArray, x::Vector{T}) where {T}
            ...
            n = Dict(nid => zeros(T,2) for nid in S)
            t = Dict(nid => zeros(T,2) for nid in S)
            gn = Dict(nid => zero(T) for nid in S)
            fc = Dict(nid => zeros(T,2) for nid in union(S,M))
            ...
            for slave_element in slave_elements
                ...
                if problem.properties.dual_basis # construct dual basis
                    ...
                    for master_element in master_elements
                        ...
                        # @debug("Contact segment found, l = $l")
                        nsegments += 1
                        for (weight, xi) in quadrature_points
                            detj = norm(0.5*(x[cs2]-x[cs1]))
                            w = weight*detj*l
                    ...
                    # @debug("Dual basis coefficient matrices", De, Me)
                    Ae = De*inv(Me)
                ...
                # Dual basis construct, create interface
            ...
            for j in union(slave_nodes, master_nodes)
                ...
                @debug("write fc for node $j to dofs $dofs")
                y[dofs] = fc[j]
            end
            ...
            # y[3:4] = is_active[node_id] ? [-gn[node_id], lat[node_id]] : la[node_id]
            # for j in S
            #     dofs = dof1, dof2 = [2*(nodemap[j]-1)+i for i=1:2]
            #     if is_active[j]
            #         y2[dof1] -= gn[j]
            #         y2[dof2] += lat[j]
            #     else
